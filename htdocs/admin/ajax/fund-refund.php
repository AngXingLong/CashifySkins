<?php
	...
	if(!empty($_POST['refund']) && !empty($_POST['invoice_id'])){
		...
		$invoice = select("select ct.steamid, ct.amount, ct.node_id, ct.node, u.credit as fund_balance from cash_transaction ct inner join user u where ct.id = ? and ct.type = 1",array($invoice_id));
